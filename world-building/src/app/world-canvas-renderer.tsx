...
[CONTENT]
Contenido en formato HTML (usa <b> para negritas, <i> para cursiva, <span style="color:..."> para colores, listas con -, etc.). ¡No uses asteriscos para formato!
[/CONTENT]`;
                  ...
                  {showDetailedZoom ? (
                    <>
                      ...
                      {(() => {
                          ...
                          const handleKeyDown = (e: React.KeyboardEvent<any>, blockIdx: number) => {
                             const block = blocks[blockIdx];
                             if (e.key === "/") {
                              ...
                              {block.type === "paragraph" && (
                                <div
                                    ...
                                    onInput={(e) => {
                                      ...
                                      updateBlock(originalIndex, { content: target.innerHTML });
                                      if (slashMenu && slashMenu.blockId === block.id) {
                                        const val = target.textContent || "";
                                        const sel = window.getSelection();
                                        const cursor = sel ? sel.anchorOffset : 0;
                                        const beforeCursor = val.substring(0, cursor);
                                        const slashMatch = beforeCursor.match(/(?:^|\s)\/(.*)$/);
                                        if (slashMatch) {
                                          setSlashMenu({ ...slashMenu, query: slashMatch[1] });
                                        } else {
                                          setSlashMenu(null);
                                        }
                                      }
                                    ...
                                    className="flex-1 bg-transparent border-none p-1 focus:bg-neutral-850/50 focus:outline-none focus:ring-1 focus:ring-link/50 rounded transition resize-none overflow-hidden min-h-[1.5em] whitespace-pre-wrap cursor-text empty:before:content-['Type_text_or_use_+_to_insert_different_block_styles...'] empty:before:text-neutral-500"
